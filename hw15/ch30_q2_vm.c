...
static bool isStringValue(Value value) {
  return IS_SHORT_STRING(value) || IS_STRING(value);
}

static int stringLength(Value value) {
  if (IS_SHORT_STRING(value)) return (int)AS_SHORT_STRING_LENGTH(value);
  return AS_STRING(value)->length;
}

static void writeStringChars(Value value, char* dest) {
  if (IS_SHORT_STRING(value)) {
    memcpy(dest, AS_SHORT_STRING_CHARS(value),
           AS_SHORT_STRING_LENGTH(value));
  } else {
...
//< Garbage Collection concatenate-peek
  ...
  if (length <= SHORT_STRING_MAX) {
    ...
    writeStringChars(a, chars);
    writeStringChars(b, chars + aLength);
//> Garbage Collection concatenate-pop
    pop();
    pop();
//< Garbage Collection concatenate-pop
    push(SHORT_STRING_VAL(chars, length));
  } else {
...
//> Strings add-strings
      case OP_ADD: {
        if (isStringValue(peek(0)) && isStringValue(peek(1))) {
          concatenate();
