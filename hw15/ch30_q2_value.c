...
#ifndef NAN_BOXING
static bool isHeapString(Value value) {
  return IS_OBJ(value) && AS_OBJ(value)->type == OBJ_STRING;
}

static bool stringValuesEqual(Value a, Value b) {
  int aLength = IS_SHORT_STRING(a)
      ? (int)AS_SHORT_STRING_LENGTH(a)
      : AS_STRING(a)->length;
  int bLength = IS_SHORT_STRING(b)
      ? (int)AS_SHORT_STRING_LENGTH(b)
      : AS_STRING(b)->length;

  if (aLength != bLength) return false;

  const char* aChars = IS_SHORT_STRING(a)
      ? AS_SHORT_STRING_CHARS(a)
      : AS_STRING(a)->chars;
  const char* bChars = IS_SHORT_STRING(b)
      ? AS_SHORT_STRING_CHARS(b)
      : AS_STRING(b)->chars;

  return memcmp(aChars, bChars, aLength) == 0;
}
#endif
...
//> Types of Values print-value
  switch (value.type) {
    ...
    case VAL_SHORT_STRING:
      printf("%.*s", (int)AS_SHORT_STRING_LENGTH(value),
             AS_SHORT_STRING_CHARS(value));
      break;
...
#else
//< Optimization values-equal
  if ((IS_SHORT_STRING(a) && isHeapString(b)) ||
      (isHeapString(a) && IS_SHORT_STRING(b))) {
