...
//< Calls and Functions call
static bool callMethod(ObjClosure* closure, int argCount,
...
static bool findMethod(ObjClass* klass, ObjString* name,
                       ObjClass** owner, Value* method) {
  if (!tableGet(&klass->dispatch, name, method)) return false;

  int depth = 0;
  for (ObjClass* current = klass;
       current != NULL;
       current = current->superclass) {
    depth++;
  }

  ObjClass* lineage[depth];
  ...
  for (int i = 0; i < depth; i++) {
    if (tableGet(&lineage[i]->methods, name, &candidate)) {
      *owner = lineage[i];
      return true;
    }
  }
...
//> Methods and Initializers call-init
        Value initializer;
        ObjClass* owner;
        if (findMethod(klass, vm.initString, &owner,
                       &initializer)) {
          return callMethod(AS_CLOSURE(initializer), argCount,
                            owner, vm.initString);
//> no-init-arity-error
        } else if (argCount != 0) {
...
static bool invokeFromClass(ObjClass* klass, ObjString* name,
  ...
  Value method;
  ObjClass* owner;
  if (!findMethod(klass, name, &owner, &method)) {
...
//< Methods and Initializers bind-method
static bool callInner(int argCount) {
  CallFrame* frame = &vm.frames[vm.frameCount - 1];
  if (frame->methodName == NULL || frame->owner == NULL) {
    runtimeError("Can't use 'inner' here.");
    return false;
  }
  ...
  while (current != NULL) {
    depth++;
    if (current == frame->owner) break;
    current = current->superclass;
  }

  if (current == NULL) {
    runtimeError("Invalid inner dispatch.");
    return false;
  }

  ObjClass* lineage[depth];
  current = instance->klass;
  for (int i = depth - 1; i >= 0; i--) {
    lineage[i] = current;
    current = current->superclass;
  }
  ...
  for (int i = 1; i < depth; i++) {
    if (tableGet(&lineage[i]->methods, frame->methodName, &method)) {
      return callMethod(AS_CLOSURE(method), argCount,
                        lineage[i], frame->methodName);
    }
  }
...
static void defineMethod(ObjString* name) {
  ...
  Value ignored;
  if (!tableGet(&klass->dispatch, name, &ignored)) {
    tableSet(&klass->dispatch, name, method);
  }
  pop();
}
//< Methods and Initializers define-method
...
//< inherit-non-class
        ...
        tableAddAll(&AS_CLASS(superclass)->dispatch,
                    &subclass->dispatch);
        pop(); // Subclass.
        break;
      }
