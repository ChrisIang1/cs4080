...
//< Global Variables identifier-constant
static Compiler* scriptCompiler() {
...
//< Local Variables identifiers-equal
static void registerGlobal(Token name, bool isMutable) {
  Compiler* compiler = scriptCompiler();

  for (int i = compiler->globalCount - 1; i >= 0; i--) {
    if (identifiersEqual(&name, &compiler->globals[i].name)) {
      compiler->globals[i].isMutable = isMutable;
      return;
    }
  }

  if (compiler->globalCount == UINT8_COUNT) {
    error("Too many global variables in script.");
    return;
  }

  compiler->globals[compiler->globalCount].name = name;
  compiler->globals[compiler->globalCount].isMutable = isMutable;
  compiler->globalCount++;
}

static bool isMutableGlobal(Token* name) {
  Compiler* compiler = scriptCompiler();

  for (int i = compiler->globalCount - 1; i >= 0; i--) {
    if (identifiersEqual(name, &compiler->globals[i].name)) {
      return compiler->globals[i].isMutable;
    }
  }
...
//< named-variable-can-assign
    if (getOp == OP_GET_LOCAL && !current->locals[arg].isMutable) {
      error("Can't assign to a const local variable.");
    } else if (getOp == OP_GET_GLOBAL && !isMutableGlobal(&name)) {
      error("Can't assign to a const global variable.");
    }
    expression();
...
static void varDeclaration(bool isMutable) {
  ...
  if (current->scopeDepth == 0) {
    registerGlobal(parser.previous, isMutable);
  }
