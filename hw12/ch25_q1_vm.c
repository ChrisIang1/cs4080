...
static bool call(ObjFunction* function, ObjClosure* closure,
                 int argCount) {
//< Closures call-signature
...
static bool callClosure(ObjClosure* closure, int argCount) {
  return call(closure->function, closure, argCount);
}
...
static bool callCallable(Value callable, int argCount) {
  ...
  switch (OBJ_TYPE(callable)) {
    ...
    case OBJ_FUNCTION:
      return callFunction(AS_FUNCTION(callable), argCount);
    default:
      runtimeError("Can only call functions and classes.");
      return false;
  }
...
//< store-receiver
        return callCallable(bound->method, argCount);
      }
//< Methods and Initializers call-bound-method
...
//> Methods and Initializers call-init
        ...
        if (tableGet(&klass->methods, vm.initString,
                     &initializer)) {
          return callCallable(initializer, argCount);
//> no-init-arity-error
        } else if (argCount != 0) {
...
static bool invokeFromClass(ObjClass* klass, ObjString* name,
  ...
  return callCallable(method, argCount);
}
//< Methods and Initializers invoke-from-class
...
//> Closures interpret-get-upvalue
      case OP_GET_UPVALUE: {
        ...
        if (frame->closure == NULL) {
          runtimeError("Missing closure for upvalue.");
          return INTERPRET_RUNTIME_ERROR;
        }
