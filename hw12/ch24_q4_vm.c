...
static bool nativeError(Value* args, const char* message) {
  args[-1] = OBJ_VAL(copyString(message, (int)strlen(message)));
  return false;
}
...
static const char* typeName(Value value) {
...
static bool lenNative(int argCount, Value* args) {
  if (!IS_STRING(args[0])) {
    return nativeError(args, "len() expects a string.");
  }
...
static bool sqrtNative(int argCount, Value* args) {
  if (!IS_NUMBER(args[0])) {
    return nativeError(args, "sqrt() expects a number.");
  }

  double value = AS_NUMBER(args[0]);
  if (value < 0) {
    return nativeError(args, "sqrt() expects a non-negative number.");
  }
  ...
  double guess = value < 1 ? 1 : value;
  for (int i = 0; i < 20; i++) {
    guess = (guess + value / guess) / 2;
  }

  args[-1] = NUMBER_VAL(guess);
  return true;
}
...
static bool strNative(int argCount, Value* args) {
  ...
  if (IS_BOOL(args[0])) {
    text = AS_BOOL(args[0]) ? "true" : "false";
    length = (int)strlen(text);
  } else if (IS_NIL(args[0])) {
    text = "nil";
    length = 3;
  } else if (IS_NUMBER(args[0])) {
    length = snprintf(buffer, sizeof(buffer), "%g", AS_NUMBER(args[0]));
    text = buffer;
  } else {
    text = typeName(args[0]);
    length = (int)strlen(text);
  }

  args[-1] = OBJ_VAL(copyString(text, length));
  return true;
}

static bool typeNative(int argCount, Value* args) {
  const char* name = typeName(args[0]);
  args[-1] = OBJ_VAL(copyString(name, (int)strlen(name)));
  return true;
}
//< Calls and Functions clock-native
//> reset-stack
...
static void defineNative(const char* name, int arity,
                         NativeFn function) {
...
//> call-native
      case OBJ_NATIVE: {
        if (argCount != AS_NATIVE_ARITY(callee)) {
          runtimeError("Expected %d arguments but got %d.",
                       AS_NATIVE_ARITY(callee), argCount);
          return false;
        }
        ...
        if (!native(argCount, args)) {
          if (IS_STRING(args[-1])) {
            runtimeError("%s", AS_CSTRING(args[-1]));
          } else {
