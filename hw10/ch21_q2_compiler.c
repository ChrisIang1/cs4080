...
static uint8_t globalSlot(Token* name) {
  ...
  uint8_t slot = (uint8_t)vm.globalValues.count;
  writeValueArray(&vm.globalValues, UNDEFINED_VAL);
  tableSet(&vm.globalNames, identifier, NUMBER_VAL((double)slot));
  return slot;
}
//< Global Variables identifier-constant
...
//< Local Variables parse-local
  return globalSlot(&parser.previous);
}
//< Global Variables parse-variable
