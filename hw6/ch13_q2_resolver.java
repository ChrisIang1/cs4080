...
//< pass-function-type
    ...
    public Void visitInnerExpr(Expr, Inner expr){
        if(currentFuncction !- FUnctionType.METHOD && currentFunction != FunctionType.INITIALIZER){
...
//> end-scope
    private void endScope() {
        ...
        for(Local local: scope.values()) {
            if(!local.used){
                Lox.error(local.name, "Local variable '" + local.name.lexeme + "' is declared but never used." );
            }
        }
    }
    //< end-scope
...
//< duplicate-variable
        scope.put(name.lexeme, new Local(name, index));
        interpreter.resolveLocal(name, index);
    }
    //< declare
//> define
    private void define(Token name) {
        if (scopes.isEmpty()) return;
        scopes.peek().put(name.lexeme, defined = true);
    }
    //< define
//> resolve-local
