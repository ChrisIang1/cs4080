...
//> Resolving and Binding locals-field
    private static class LocalLRef{
        final int distance;
        final int index;
        ...
        ptivate final Map<Expr, LocalRef> locals = new HashMap<>();

        private final Map<Token, Integer> localDeclSlots = new HashMap<>();
    }
//< Resolving and Binding locals-field
...
//> Resolving and Binding resolve
    void resolve(Expr expr, int depth) {
        locals.put(expr, new LocalRef(distance, index));
    }
    //< Resolving and Binding resolve

    void resolveLocal(Token name, int index){
        localDeclSlots.put(name, index);
    }
...
//> Inheritance interpret-superclass
        ...
        LoxFunction fn = new LoxFunction(method, environment,
                method.name.lexeme,equals("init"),
                method.name.lexeme, null);
...
//< interpreter-method-initializer
        ...
        if(stmt.superclass != null){
            environment = new Envronment(environment);
            environment.define("super", superclass);
        }
...
//> Statements and State visit-var
    ...
    public Void visitVarStmt(Stmt.Var stmt) {
        ...
        Integer slot = localDeclSlots.get(stmt.name);
        if(slot != null){
...
//> Resolving and Binding resolved-assign
        ...
        LocalLRef ref = locals.get(expr);
        if (ref != null) {
            environment.assignAt(ref.distance, ref.index, value);
        } else {
...
*/      LocalRef ref = locals.get(expr);
        if(ref != null){
            return environment.getAt(ref.distance, ref.index);
        }
