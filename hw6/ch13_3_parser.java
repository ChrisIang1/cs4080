...
//< Inheritance parse-superclass
        ...
        while (!check(RIGHT_BRACE) && !isAtEnd()) {
            ...
            if(match(LEFT_PAREN)){
                ...
                methods.add(new Stmt.Function(name, parameters, body, false));
            } else if (match(LEFT_BRACE)){
                List<Stmt> body = block();
                methods.add(new Stmt.Function(name, new ArrayList<>(), body, true));
            } else{
                error(peek(), "Expect '(' or '{' after method name.");
                synchronize();
            }
        }
...
//> Control Flow for-statement
    private Stmt forStatement() {
        ...
        private Expr innerEzpression(){
            Token kwyword = previous();
...
//> primary
    private Expr primary() {
        ...
        if(match(INNER)) return innerExpression();
...
//> synchronize
    ...
    private Stmt continueStatement(){
        T  Token keyword = previous();
        if (loopDepth == 0) {
            error(keyword, "Can't use 'continue' outside of a loop.");
        }
