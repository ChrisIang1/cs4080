...
#define INITIAL_SEMISPACE_SIZE (1024 * 1024)
...
//> Garbage Collection init-gc-fields
  vm.bytesAllocated = 0;
  vm.nextGC = INITIAL_SEMISPACE_SIZE;
  vm.heapCapacity = INITIAL_SEMISPACE_SIZE;
  vm.fromSpace = (uint8_t*)malloc(vm.heapCapacity);
  vm.toSpace = (uint8_t*)malloc(vm.heapCapacity);
  if (vm.fromSpace == NULL || vm.toSpace == NULL) exit(1);
  vm.allocPtr = vm.fromSpace;
//< Garbage Collection init-gc-fields
