...
//< Calls and Functions clock-native
static void ensureStackCapacity(int needed) {
  if (vm.stackCapacity >= needed) return;

  int oldCapacity = vm.stackCapacity;
  Value* oldStack = vm.stack;
  int used = oldStack == NULL ? 0 : (int)(vm.stackTop - vm.stack);

  int newCapacity = oldCapacity < INITIAL_STACK_CAPACITY
  ...
  vm.stackTop = vm.stack + used;

  if (oldStack == NULL || oldStack == vm.stack) return;
  ...
  for (ObjUpvalue* upvalue = vm.openUpvalues;
    ...
    if (upvalue->location >= oldStack &&
        upvalue->location < oldStack + used) {
      upvalue->location += delta;
    }
  }
}
//> reset-stack
static void resetStack() {
  ensureStackCapacity(INITIAL_STACK_CAPACITY);
